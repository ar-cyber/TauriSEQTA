<script lang="ts">
  ...
  function getCommandIcon(title: string): string {
    const icons: Record<string, string> = {
      ...
      Text: '<svg class="w-5 h-5 text-slate-300" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M4 4a2 2 0 012-2h8a2 2 0 012 2v12a2 2 0 01-2 2H6a2 2 0 01-2-2V4zm2 0v12h8V4H6z" clip-rule="evenodd"></path><path d="M8 6h4M8 8h4M8 10h2"></path></svg>',
      Quote:
      ...
      Divider:
        '<svg class="w-5 h-5 text-slate-400" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M3 10a1 1 0 011-1h12a1 1 0 110 2H4a1 1 0 01-1-1z" clip-rule="evenodd"></path></svg>',
      'Bold Text':
    ...
    return (
      icons[title] ||
      '<svg class="w-5 h-5 text-slate-400" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clip-rule="evenodd"></path></svg>'
    );
  }
</script>
    ...
    {#each $slashItems as { title, subtitle, command }, i}
      <div
        class="p-2 flex gap-3 cursor-pointer {i == $selectedIndex &&
          'dark:bg-slate-950/50 bg-slate-300/50'} dark:hover:bg-slate-950/30 hover:bg-slate-300/20 rounded-lg mx-2"
