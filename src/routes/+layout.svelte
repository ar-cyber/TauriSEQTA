...
<div class="flex flex-col h-screen">
  ...
  <header class="flex justify-between items-center px-6 w-full h-16">
    <div class="flex items-center space-x-4">
      <button
        class="flex justify-center items-center w-10 h-10 bg-gray-100 rounded-xl transition-all duration-200 hover:bg-gray-200 dark:bg-slate-800 dark:hover:bg-slate-700 focus:outline-none focus:ring-2 focus:ring-gray-500/50"
        onclick={() => (sidebarOpen = !sidebarOpen)}
        aria-label="Toggle sidebar"
      >
        <Icon src={Bars3} class="w-5 h-5 text-gray-700 dark:text-gray-300" />
      </button>
      <div class="flex items-center space-x-3">
        <img
          src="/betterseqta-dark-icon.png"
          alt="DesQTA"
          class="w-8 h-8 invert dark:invert-0"
        />
        <h1
          class="text-xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-gray-900 to-gray-600 dark:from-white dark:to-gray-300"
        >
          DesQTA
        </h1>
      </div>
      {#if weatherEnabled && weatherData}
        <div
          class="flex gap-3 items-center px-4 py-2 ml-4 text-sm font-medium rounded-xl border backdrop-blur-md bg-white/60 border-gray-200/40 dark:bg-slate-800/60 dark:border-slate-700/40"
        >
          ...
          <span class="hidden text-gray-600 sm:inline dark:text-gray-400"
    ...
    <div class="flex items-center space-x-2">
      <button
        class="flex relative justify-center items-center rounded-xl border transition-all duration-200 size-12 bg-white/60 border-gray-200/40 hover:bg-white/80 dark:bg-slate-800/60 dark:border-slate-700/40 dark:hover:bg-slate-800/80 focus:outline-none focus:ring-2 focus:ring-gray-500/50"
        onclick={() => (notifications = [])}
      >
        <Icon src={Bell} class="w-5 h-5 text-gray-700 dark:text-gray-300" />
      ...
      {#if userInfo}
        <div class="relative user-dropdown-container">
          <button
            class="flex gap-3 items-center px-4 py-2 rounded-xl border transition-all duration-200 bg-white/60 border-gray-200/40 hover:bg-white/80 dark:bg-slate-800/60 dark:border-slate-700/40 dark:hover:bg-slate-800/80 focus:outline-none focus:ring-2 focus:ring-gray-500/50"
          ...
          >
            ...
            />
            <span
              class="hidden font-semibold text-gray-900 md:inline dark:text-white"
            >
          ...
          {#if showUserDropdown}
            ...
            >
              <div class="p-2">
                ...
                <div
                  class="my-2 border-t border-gray-200 dark:border-slate-700/40"
                ></div>
  ...
  <div class="flex flex-1 min-h-0">
    ...
    >
      <nav class="p-4 py-[1px] space-y-2 w-64 min-w-64">
        {#each menu as item}
          <a
            href={item.path}
            class="flex gap-4 items-center text-md px-3 py-3 font-medium rounded-xl transition-all duration-200 hover:bg-gray-100/50 dark:hover:bg-slate-800/50 focus:outline-none {(
              item.path === '/'
                ? $page.url.pathname === '/'
                : $page.url.pathname.startsWith(item.path)
            )
              ? 'bg-gray-100/50 ring-1 ring-gray-300/40 text-gray-900 dark:bg-slate-800/50 dark:ring-slate-700/50 dark:text-white'
    ...
    <!-- Main Content -->
    <main
      class="overflow-y-auto flex-1 rounded-tl-2xl border-t border-l border-gray-200 bg-slate-100/50 dark:border-slate-700/50 dark:bg-slate-900/50"
    >
      {#if $needsSetup}
        <div
          class="flex justify-center items-center p-6 h-full bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50 dark:from-slate-900 dark:via-slate-800 dark:to-slate-900"
        >
          <div
            class="flex overflow-hidden flex-col w-full max-w-6xl rounded-3xl border shadow-2xl backdrop-blur-xl bg-white/80 border-gray-200/60 dark:bg-slate-900/80 dark:border-slate-700/60 md:flex-row animate-fade-in-up"
          ...
          >
            ...
            <div class="hidden relative md:block md:w-2/3">
              ...
              >
                ...
                <p class="text-lg text-gray-200">
                  Experience SEQTA Learn like never before with our powerful
                  desktop application
                </p>
              </div>
            </div>
            ...
            <div
              class="flex flex-col justify-center p-8 w-full md:w-1/3 md:p-12"
            >
                ...
                <div>
                  ...
                  <div class="relative">
                    ...
                    <input
                      ...
                      class="py-3 pr-4 pl-10 w-full text-gray-900 bg-gray-50 rounded-lg border border-gray-300 transition-colors dark:bg-slate-800 dark:text-white dark:border-slate-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
                    />
                  </div>
