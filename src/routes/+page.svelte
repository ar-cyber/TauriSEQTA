		...
		<div class="flex gap-4 overflow-x-auto px-4 py-6">
		  {#each lessons as lesson, i}
			<div class="relative flex flex-col min-w-[170px] max-w-[210px] bg-[var(--surface-alt)] rounded-xl shadow-lg border-l-8 transition-transform duration-300 hover:scale-105" style="border-color: {lesson.colour};">
			  <div class="px-4 pt-4 pb-2 flex flex-col gap-1 flex-1">
				<div class="flex items-center justify-between">
				  <span class="font-bold text-base" style="color: var(--text);">{lesson.description}</span>
				  {#if lesson.active}
					<span class="ml-2 px-2 py-0.5 rounded bg-green-600 text-xs text-white">Now</span>
				  {/if}
				</div>
			  ...
			  {#if lesson.programmeID !== 0}
				<div class="flex gap-2 px-4 pb-3 pt-1">
				  <button class="hover:scale-110 transition-transform duration-300" aria-label="View Assessment" onclick={() => (location.href = buildAssessmentURL(lesson.programmeID, lesson.metaID))}>
				  ...
				  <button class="hover:scale-110 transition-transform duration-300" aria-label="View Course" onclick={() => (location.href = `../#?page=/courses/${lesson.programmeID}:${lesson.metaID}`)}>
